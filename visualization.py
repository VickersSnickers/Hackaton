...
plt.rcParams["axes.unicode_minus"] = False  # чтобы минусы в осях не превращались в квадраты
...
# Извлекаем год и фильтруем ненужные категории
df["Год"] = df["Публикации"].str.extract(r"(\d{4})")
df = df[(df["Год"].notna()) & (df["Область"] != "Не классифицировано")]

# Взрываем список авторов для группировок
...
author_area_counts = (
    df_exploded.groupby(["Авторы", "Область"])
    .size()
    .unstack(fill_value=0)
)
...
def save_plot(fig, name):
    """Сохраняет график с автоинкрементом имени"""
...
# ===============================
top_20_authors = heatmap_data.sum(axis=1).nlargest(10).index
heatmap_top = heatmap_data.loc[top_20_authors]

fig, ax = plt.subplots(figsize=(14, 8))
sns.heatmap(heatmap_top, cmap="YlGnBu", linewidths=0.5, annot=True, fmt="d", ax=ax)
ax.set_title("Тепловая карта: Авторы × Годы (кол-во публикаций)")
ax.set_xlabel("Год")
ax.set_ylabel("Автор")
plt.tight_layout()
...
# ===============================
# 5. Годы × Области (стековая гистограмма)
# ===============================
fig, ax = plt.subplots(figsize=(14, 7))
year_area_counts.plot(
    kind="bar",
    stacked=True,
    colormap="tab20",
    ax=ax
...
ax.set_title("Тепловая карта: Количество патентов по областям и годам")
ax.set_xlabel("Год")
